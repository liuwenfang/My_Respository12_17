...
public abstract class ZRecyclerViewAdapter<T> extends BaseRecyclerViewAdapter<T, ZViewHolder> {
    ...
    public ZRecyclerViewAdapter(@NonNull RecyclerView mRecyclerView, List<T> dataLists) {
        this(mRecyclerView, dataLists, -1);
    }

    public ZRecyclerViewAdapter(@NonNull RecyclerView mRecyclerView, List<T> dataLists, @LayoutRes int layoutId) {
    ...
    public ZViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (isHeaderViewType(viewType)) {
            return new ZViewHolder(mContext, mHeaderViews.get(viewType));
        }

        if (isFooterViewType(viewType)) {
            return new ZViewHolder(mContext, mFooterViews.get(viewType));
        }
        if (viewType == TYPE_NO_MORE) {
            mNoMoreView = inflater.inflate(R.layout.adapter_no_more, mRecyclerView, false);
            return new ZViewHolder(mContext, mNoMoreView);
        }
        if (viewType == TYPE_LOAD_MORE) {
            mLoadMoreView = inflater.inflate(R.layout.adapter_loading, mRecyclerView, false);
            return new ZViewHolder(mContext, mLoadMoreView);
        }
        if (viewType == TYPE_LOAD_FAILED) {
            mLoadMoreFailedView = inflater.inflate(R.layout.adapter_loading_failed, mRecyclerView, false);
            return new ZViewHolder(mContext, mLoadMoreFailedView);
        }
        return new ZViewHolder(mContext, inflater.inflate(viewType, parent, false));
    }

    @Override
    public void onBindViewHolder(final ZViewHolder holder, int position) {
    ...
    protected abstract void bindData(ZViewHolder holder, T data, int position);
    ...
    @Override
    public void onViewAttachedToWindow(ZViewHolder holder) {
    ...
    public ZRecyclerViewAdapter<T> setOnLoadMoreListener(OnLoadMoreListener onLoadMoreListener) {
        this.onLoadMoreListener = onLoadMoreListener;
        return this;
    }

    public ZRecyclerViewAdapter<T> setOnItemClickListener(OnItemClickListener onItemClickListener) {
        this.onItemClickListener = onItemClickListener;
        return this;
    }

    public ZRecyclerViewAdapter<T> setOnItemLongClickListener(OnItemLongClickListener onItemLongClickListener) {
