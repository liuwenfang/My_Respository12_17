...
public abstract class JsonCallback<T> extends AbsCallback<T> {
    ...
    public JsonCallback(Context mContext, String msg) {
        this.mContext = mContext;
        showProDialog(msg);
    }
    ...
    private void showProDialog(String msg) {
        ...
        if (loadingDialog == null) {
            builder = new LoadingDialog.Builder(mContext)
                    .setCancelOutside(true).setShowMessage(true);
        }
...
//        boolean isLogin = AppContext.getInstance().checkUser();
//        request.headers("role", "washUser");
//        if (isLogin) {
//            request.headers("token", AppContext.getInstance().getUserInfo().getToken());
//        }
    }
    ...
    public void onError(com.lzy.okgo.model.Response<T> response) {
        ...
        if (response.getException().getMessage().contains("用户信息不存在")) {
            if (mContext != null) {
                ToastUtils.showTextToast(mContext, "用户信息已过期,请重新登录!");
                AppContext.getInstance().cleanUserInfo();
                EventBus.getDefault().post(new CommonBean<>("logOverdue"));
